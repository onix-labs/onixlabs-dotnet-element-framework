...
- **`Neo4jCypherExecutor` materializes rows eagerly.** `IAsyncEnumerable` is a façade — the cursor is drained before the executor returns. Queries returning millions of rows will OOM. Design choice, not a bug.
...
- **Read aliases are tribal.** `EmitFindById` and `EmitAsEnumerableNodes` project entities under `"n"`; `EmitAsEnumerableEdges` under `"r"`; `EmitExists` returns a `"count"` column. New providers must honour this. See [architecture.md §6](architecture.md#6-provider-contract).
